...
def process_log_file(cur, filepath):
    """Process input data (1 JSON file) and insert into users, time and
        songplays tables.
    Keyword arguments:
    * cur --        reference to connected db.
    * filepath --   path to file to be processed.
    ...
    * songplay_data in songplay table.
    ...
    # load user table
    user_data = df.get(['userId','firstName','lastName','gender','level'])
...
def main():
    ...
    * All input data processed in DB tables.
    ...
    conn = psycopg2.connect(
        "host=127.0.0.1 dbname=sparkifydb user=postgres password=login@123")
    cur = conn.cursor()
